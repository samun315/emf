                    ...
                    <form class="form" method="POST" id="kt_project_form" enctype="multipart/form-data"
                        ...
                        <div class="row mb-5">
                            ...
                            <div class="col-md-6 fv-row mb-5">
                                <label class="fs-5 fw-bold mb-2">Project/Program Name</label>
                                <select name="project_or_program_name"
                                    class="form-select form-select-solid @error('project_or_program_name') is-invalid @enderror"
                                    data-control="select2" data-placeholder="Select Project/Program Name">

                                    @foreach ($projects as $project)
                                        <option
                                            {{ isset($editModeData) && $editModeData->project_id == $project->id ? 'selected' : '' }}
                                            value="project_{{ $project->id }}">{{ $project->title }}
                                        </option>
                                    @endforeach

                                    @foreach ($programs as $program)
                                        <option
                                            {{ isset($editModeData) && $editModeData->program_name == $program->id ? 'selected' : '' }}
                                            value="program_{{ $program->id }}">{{ $program->name }}
                                        </option>
                                    @endforeach

                                </select>
                                @error('project_or_program_name')
                            ...
                            <div class="col-md-12 fv-row mb-5">
                                ...
                                <textarea class="form-control form-control-solid" placeholder="Enter message" name="message"
                                    data-kt-autosize="true">{{ $editModeData->message ?? old('message') }}</textarea>
                                @error('message')
                                    <span class="text-danger mt-2">{{ $message }}</span>
                                @enderror
                            </div>
